...
GMSMapViewDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    private func setCountry(withName name: String) {
        ...
        geoService.getCountry(byCode: pair.key) { data, _ in
    ...
    private func setCountry(byCoord coord: CLLocationCoordinate2D) {
        ...
        setUserPeekIsEnable(false)
        geocoder.reverseGeocodeCoordinate(coord) { data, _ in
    ...
    private func setPickerValue(_ name: String) {
        if let index = countries.firstIndex(where: {$0 == name}) {
