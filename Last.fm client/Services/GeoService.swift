...
class GeoService {
    ...
    func getCountry(byCode code: String, closure: @escaping (CLLocationCoordinate2D, Error?) -> Void) {
        httpClient.get(urlAppend: code, contentType: .json) { data, _ in
    ...
    func getLocationRelatedTop() -> [TableViewInfo] {
        if userDefaults.getLocationState() {
            ...
            return [TableViewInfo(data: [], navName: "Top Artists",
                                  dataSource: apiService.getTopArtistsClosure(byCountry: location.name)),
                    TableViewInfo(data: [], navName: "Top Tracks",
                                  dataSource: apiService.getTopTracksClosure(byCountry: location.name))]
        } else {
            return [TableViewInfo(data: [], navName: "Top Artists",
